# HALE UAV Presentation — Merged Slide Transcript
**Version 3 — Integrated Research & Personal Findings**
...
> **Alternative Estimate (Component Build-up):** Component-level analysis yields ~57–62 kg MTOW (including payload). *See Reconciliation Notes for details.*
...
**Title:** Hybrid Energy Architecture — Solar Power Feasibility Analysis

**Solar Power Analysis:**
- **Available irradiance at 20 km altitude:** ~1,360 W/m² (AM0 spectrum) *(Ref: [15])*
- **Solar array coverage:** Wing upper surface + fuselage top — approximately 45–50 m² effective area
- **Panel efficiency (triple-junction GaAs):** 25–30% *(Ref: [14][15])*
- **Practical power generation:** ~9 kW peak (daytime)
- **PV integration derating:** 10–15% losses from wiring, soiling, temperature, mismatch *(Ref: [43])*
- **Net usable solar power:** ~7.5–8 kW after derating
...
- Cell type: Triple-junction GaAs (space-grade)
- Efficiency: 25–30% (laboratory: up to 32%)
- Specific power: ~200–250 W/kg
- Array mass estimate: ~15 kg (including mounting and wiring)
- Operating temperature: -50°C to +40°C (stratospheric)
...
**Title:** Structural Analysis & Material Specifications
...
- High stiffness-to-weight ratio: **~5× aluminium** (E ~150 GPa vs ~70 GPa for Al 7075)
- Specific stiffness: 13.3 × 10⁶ m²/s² (vs 2.6 × 10⁶ for Al)
- Anisotropic tailoring for spanwise bending and torsion load paths
- Autoclave/RTM manufacturing for void content <1%
- Laminate design: Quasi-isotropic [0/45/90/-45]ns for skins; unidirectional for caps

**Right Column — Structural Analysis Methods** *(Refs: [5][22][44])*
- FEA tool: ANSYS Workbench, large-displacement formulation
- Static deflection: expected tip deflection **10–15% of semi-span** (benchmark: EAV-3 shows 404 mm at 1-G) *(Ref: [40])*
- Modal frequency analysis: First bending ~2–3 Hz; first torsion ~5–7 Hz
- Flutter analysis: Murua et al. UVLM methodology for geometrically exact beams + free-wake coupling *(Ref: [44])*
- Safety factor: 1.5 on ultimate load; 2.0 on fatigue

**Structural Analysis Results:**
- Wing root bending moment (1-G cruise): ~15,000 Nm
- Wing tip displacement estimate: 2.4–3.6 m (10–15% of 16 m semi-span)
- First natural frequency: ~2.5 Hz (bending)
- Flutter speed margin: Target 1.3× operating speed
- Composite failure index: Tsai-Wu criterion < 1.0
...
| Energy Analysis Mass Ref. | ~114 kg | From Section 6.2 feasibility calculation | — |
| MTOW (Component Build-up) | ~57–62 kg | With max payload | — |
...
> **Mass Reconciliation Note:** A single unified MTOW figure is not finalised at conceptual stage. The component-level estimate (~57–62 kg incl. payload) and the energy-analysis estimate (~114 kg) reflect different sizing assumptions:
> - **Component build-up:** Airframe + solar + fuel cell + avionics + payload = ~57–62 kg
> - **Energy feasibility:** Includes margin for battery alternative analysis showing infeasibility
> These require reconciliation in detailed design.
...
| Max Take-off (component) | ~57–62 kg | With max payload |
...
## Reconciliation Notes – Personal Findings vs Literature

### Mass Estimate Discrepancy

**Identified Conflict:**
- **Literature-based estimate:** ~114 kg (from energy feasibility analysis in Section 6.2)
- **Component build-up estimate:** ~57–62 kg MTOW (airframe + solar + fuel cell + avionics + payload)

**Resolution:**
Both figures are retained with clear attribution:

1. **~114 kg figure:** Derived from energy balance calculations, representing the mass envelope when accounting for worst-case energy storage requirements. This figure includes margin for the battery alternative analysis that demonstrates pure-battery infeasibility.

2. **~57–62 kg figure:** Derived from component-level mass build-up using benchmark data from EAV-3 [40] and represents the actual expected operational weight with payload.

**Recommendation for Presentation:**
- Use ~57–62 kg as the primary MTOW figure for comparison with existing platforms (Zephyr S: 53 kg, EAV-3: 53 kg)
- Reference ~114 kg only in technical discussions regarding energy system sizing
- Acknowledge this discrepancy as a limitation requiring detailed mass property analysis in detailed design

### Formatting Integration

The condensed slide data from the supplementary file has been integrated to enhance slide-ready formatting while preserving all academic citations and technical depth from the primary research document.

---

*Document Version: 3.0 — Merged Transcript*
